...
export default function PresentationApp() {
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative', background: 'var(--bg-gradient)', overflow: 'hidden' }}>
      ...
      <div style={{
        position: 'absolute', top: '32px', left: '32px', zIndex: 100,
        display: 'flex', alignItems: 'center', gap: '16px', pointerEvents: 'none'
      }}>
        <div style={{ 
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: '22px', fontWeight: '700', letterSpacing: '0.5px', textShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>The Pinnacle Residence</h1>
          <p style={{ margin: '2px 0 0', color: 'var(--text-secondary)', fontSize: '12px', fontWeight: '600', letterSpacing: '2px' }}>ARCHVIZ STUDIO LTD.</p>
        </div>
      </div>

      {/* Global Controls - Top Right */}
      <div style={{ position: 'absolute', top: 32, right: 32, zIndex: 100, display: 'flex', gap: '12px' }}>
        <button 
      ...
      {/* Floating Navigation Pill - Top Center */}
      <div style={{
        position: 'absolute', top: '32px', left: '50%', transform: 'translateX(-50%)', zIndex: 100
      }}>
        <div className="glass-panel" style={{ 
          ...
          <div style={{ width: '1px', background: 'var(--border-glass-light)', margin: '0 4px' }}></div>
          {isAdmin && <TabButton active={activeTab === 'manage'} icon={Settings} label="Manage Settings" onClick={() => setActiveTab('manage')} />}
        </div>
      </div>
